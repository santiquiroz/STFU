...
# 1. Quitar el dispositivo con devcon si esta disponible
...
# 2. Borrar los paquetes de driver STFU/VirtualAudioDriver del DriverStore
...
foreach ($line in $drivers) {
    ...
    if ($line -match "Original Name\s*:\s*(.*\.inf)" -and $current) {
        if ($Matches[1] -match "VirtualAudioDriver|stfu") {
            Write-Host ("  eliminando " + $current) -ForegroundColor DarkGray
...
Write-Host "Listo. El audio del sistema vuelve a su estado normal." -ForegroundColor Green
